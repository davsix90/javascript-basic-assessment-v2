...
var placeInLine = [1,2,3,4];

cutInLine(placeInLine, 8);
...
function tripleTheFun(thing) {
  if(typeof Number(thing) == 'number' && isNaN(thing) == false) {
    return +Number(thing) + +Number(thing) + +Number(thing);
  } else {
...
console.log(typeof Number('poop'))
console.log(tripleTheFun('50'))
...
// #14 Create a function called spiderTest that takes in an object that is an actor, it looks to see if the actor's name is Andrew Garfield and changes the actor object 
// to have a property called spiderRating with a value of 10.  If their name is Tobey Maguire set spiderRating to 0.  otherwise set spiderRating to 'not a spiderman'.
